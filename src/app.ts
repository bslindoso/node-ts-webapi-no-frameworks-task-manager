...
import { getTaskById, getTasksList, postTask, unknownRoute } from "./controllers/task-manager-controller"
...
export const app = async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
  ...
  const taskId = (partsUrl.length > 2) && partsUrl[2] !== "" ? partsUrl[2] : null

  console.log(`[${request.method}] Rota "${baseurl}" => ID "${taskId}"`)

  // GET /tasks
  if (request.method === HttpMethod.GET && baseurl === Routes.LIST_TASKS && taskId === null) {
